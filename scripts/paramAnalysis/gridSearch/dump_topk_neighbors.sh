...
Env overrides:
  ...
  DIST_FN              Default: l2
  MAX_PARALLEL         Batch mode parallel workers (default: all CPU cores)
  DRY_RUN=1            Print commands only
  QUIET=1              Less logging (recommended in batch mode)
USAGE
}
...
TOPK="${TOPK:-10}"
MAX_PARALLEL="${MAX_PARALLEL:-$(nproc 2>/dev/null || getconf _NPROCESSORS_ONLN || echo 1)}"
QUIET="${QUIET:-0}"
...
if [[ -d "$INPUT_PATH" ]]; then
    ...
    echo "Found ${#expanded_files[@]} file(s), MAX_PARALLEL=${MAX_PARALLEL}"
    failed=0
    for expanded_csv in "${expanded_files[@]}"; do
        ...
        env TOPK="${TOPK}" OUTPUT_DIR="${OUTPUT_DIR:-}" BUILD_DIR="${BUILD_DIR:-}" \
            ...
            EXPERIMENT_TAG="${EXPERIMENT_TAG:-}" DRY_RUN="${DRY_RUN:-0}" QUIET=1 \
            bash "$0" "${expanded_csv}" &
    ...
    if [[ "$failed" -ne 0 ]]; then
        echo ""
        echo "ERROR: Batch completed with failures" >&2
        exit 1
    fi
    echo ""
