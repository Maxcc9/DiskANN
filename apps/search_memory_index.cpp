                        ...
                        const std::string &stats_csv_path = "")
{
    ...
    else
    {
        per_query_csv << "query_id,L,beamwidth,thread_id,total_us,io_us,cpu_us,n_ios,read_size,n_cmps,n_cache_hits,"
                      << "n_hops,expanded_nodes_count,recall_match_count\n";
    }
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        auto visited_mean = diskann::get_mean_stats<uint32_t>(
            stats.data(), query_num, [](const diskann::QueryStats &s) { return s.expanded_nodes_count; });
        auto visited_p50 = diskann::get_percentile_stats<uint32_t>(
            stats.data(), query_num, 0.5f, [](const diskann::QueryStats &s) { return s.expanded_nodes_count; });
        auto visited_p90 = diskann::get_percentile_stats<uint32_t>(
            stats.data(), query_num, 0.9f, [](const diskann::QueryStats &s) { return s.expanded_nodes_count; });
        auto visited_p95 = diskann::get_percentile_stats<uint32_t>(
            stats.data(), query_num, 0.95f, [](const diskann::QueryStats &s) { return s.expanded_nodes_count; });
        auto visited_p99 = diskann::get_percentile_stats<uint32_t>(
            stats.data(), query_num, 0.99f, [](const diskann::QueryStats &s) { return s.expanded_nodes_count; });
        ...
        if (per_query_csv.is_open())
        {
            ...
            for (uint32_t qi = 0; qi < query_num; qi++)
            {
                ...
                oss << qi << "," << L << "," << L << "," << stats[qi].thread_id << "," << stats[qi].total_us << ","
                    ...
                    << stats[qi].n_hops << "," << stats[qi].expanded_nodes_count << "," << stats[qi].recall_match_count
                    << "\n";
            }
